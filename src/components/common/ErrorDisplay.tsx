...
export default function ErrorDisplay({ error, componentStack, onReset }: ErrorDisplayProps) {
  const [showDetails, setShowDetails] = useState(false);

  // Handle React Router specific error objects
  const errorMessage = error?.message || error?.statusText || (error?.status ? `Error ${error.status}` : 'Unknown error');
  const errorDetails = error?.data || error?.stack || null;
            ...
            {showDetails && (
              <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                <div className="rounded-lg bg-gray-900 p-4 font-mono text-xs text-red-400 overflow-x-auto shadow-inner">
                  ...
                  {errorDetails && (
                    <>
                      <p className="mb-2 mt-4 font-bold uppercase tracking-wider text-gray-500">Error Details:</p>
